...
object MiHoYoAuth {
    ...
    suspend fun login(
        username: String, password: EncryptedPassword, captchaData: CaptchaData
    ) = withContext(Dispatchers.IO) {
        /* 使用密码登录，获取用户ID和登录凭据 */
        var user = with(MiHoYoAPI.loginByPassword(username to password, captchaData)) {
            val i = getAsJsonObject("account_info")
            MiAccount(
                uid = i["account_id"].asString, ticket = i["weblogin_token"].asString
            )
        }/* 通过登录凭据获取两份令牌(ltoken,stoken) */
        user = MiHoYoAPI.getMultiTokenByLoginTicket(user)/* 获取米游社个人信息(头像Url) */
        val a = async {
            runCatching {
        ...
        user.copy(
            guid = profile.uid, name = profile.name, avatar = avatar
        )
    }
    ...
    suspend fun login(
        ...
        val b = async { MiHoYoAPI.getUserGameRolesByCookie(user).getOrNull(0) }
        val (profile, avatar) = b.await() to a.await()
        checkNotNull(profile) { "空账号" }/* 创建Account对象并返回 */
        user.copy(
            guid = profile.uid, name = profile.name, avatar = avatar
        )
    }
