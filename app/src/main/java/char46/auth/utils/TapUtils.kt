...
val TapUrlRegex =
    Regex("https://www\\.taptap\\.com/qrcode/to\\?url=https%3A%2F%2Fwww\\.taptap\\.com%2Fdevice%3Fqrcode%3D1%26user_code%3D\\w{5}")
...
object TapAPI {
    ...
    private suspend inline fun <reified R : Any> TapAccount.request(url: String) =
        request(url) { toDataClass<R>() }

    private suspend inline fun <reified R : Any> TapAccount.post(
        url: String, formBuilder: FormBody.Builder.() -> Unit
    ) = FormBody.Builder().apply(formBuilder).build()
        .let { body -> request(url, body) { toDataClass<R>() } }

    private suspend fun <R> TapAccount.request(
        url: String, body: RequestBody? = null, scope: Response.() -> R
        ...
        buildHttpRequest {
            ...
            addHeader(
                "X-UA", "V=1&PN=Accounts&LANG=${acc.locale}&VN_CODE=1&UID=$uuid&VID=${acc.uid}"
            )
            addHeader(
                "X-CLIENT-XUA",
                "V=1&PN=TapTap&VN_CODE=223001000&LOC=CN&LANG=${acc.locale}}&CH=default&UID=$uuid&VID=${acc.uid}&$deviceXUA"
            )
            body?.let { post(body) }
    ...
    data class TapOAuthCode(
        ...
        @SerializedName("user_code") val user: String = "",
    ...
    private data class TapOAuthResult(
        @SerializedName("redirect_uri") val redirectTo: String
    )

    data class TapProfile(
        @SerializedName("avatar") val avatar: String = "",
        @SerializedName("nickname") val nickname: String = "",
        @SerializedName("user_id") val uid: Int = 0
    )

    private data class TapOAuth(
        @SerializedName("client") val client: Client,
        @SerializedName("continuation_code") val code: String = "",
        @SerializedName("grant_type") val type: String = ""
    ) {
        data class Client(
    ...
    private inline fun <reified T : Any> Response.toDataClass() =
        getStringBody().toJsonObject().let { obj ->
