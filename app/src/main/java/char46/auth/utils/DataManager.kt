...
import char46.auth.security.asEncryptedFile
...
data class DeprecatedTapAccount(
    @SerializedName("c") override val uid: String = "",
    @SerializedName("i") override val name: String = "null"
) : IAccount(uid, name, "")

private fun migrate() {
    val oldTapAccounts =
        dataDir.listFiles()?.filter { f -> f.nameWithoutExtension.startsWith("t_") }
    oldTapAccounts?.forEach {
        removedTapAccounts.add(
            gson.fromJson(
                it.asEncryptedFile().readText(), DeprecatedTapAccount::class.java
            )
        )
        it.delete()
    }
    ...
    if (df?.isNotEmpty() == true) {
        df.forEach {
            ...
            File(dataDir, it.nameWithoutExtension.removePrefix("decrypted_")).asEncryptedFile()
