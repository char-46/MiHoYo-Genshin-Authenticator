...
import char46.auth.activities.TapAuthActivity
...
import char46.auth.utils.TapAPI
import char46.auth.utils.accountList
import char46.auth.utils.addTo
import char46.auth.utils.cookieStringToMap
import char46.auth.utils.decryptAll
import char46.auth.utils.exists
import char46.auth.utils.ioScope
import char46.auth.utils.refreshAccount
import char46.auth.utils.removedTapAccounts
import char46.auth.utils.toast
import char46.auth.utils.ui.CircularProgressDialog
...
fun MainActivity.UI() {
    ...
    TopAppBar(a = lst.size, normalDropdownItems = buildDropdownMenuItems {
        add("米哈游账号登录") {
        ...
        add("解密账号数据") {
            ...
            ioScope.launch {
                ...
                showAlertDialog(
                    "解密完毕",
                    "你现在可以进行数据迁移了！下一次打开应用时，所有的账号信息将会被重新加密"
                )
            }
        }
    ...
    ioScope.launch {
        ...
        runCatching {
            val account = with(cookieStringToMap(s)) {
                TapAccount(
                    locale = getOrDefault("locale", "zh_CN"), session = getValue("ACCOUNTS_SESS")
                )
            }
            ...
            with(TapAPI) {
                ...
                account.copy(
                    name = p.nickname, avatar = p.avatar, uid = p.uid.toString()
                )
            } addTo accountList
...
fun MainActivity.NormalAlertDialog() = run {
    if (isAlertDialogShowing) AlertDialog(onDismissRequest = ::hideAlertDialog, title = {
