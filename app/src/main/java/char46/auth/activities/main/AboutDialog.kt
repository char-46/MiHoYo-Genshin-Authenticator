...
@ExperimentalMaterialApi
@Composable
private fun MainActivity.AD() = Dialog(onDismissRequest = { isDialogShowing = false }) {
    Column(
...
private fun MainActivity.joinQQGroup(key: String) = runCatching {
    val url = URLEncoder.encode(
        "http://qm.qq.com/cgi-bin/qm/qr?from=app&p=android&jump_from=webapi&k=", "utf-8"
    )
    Intent().let {
...
private fun Component(
    title: String, text: String, onClick: () -> Unit
) = Component(AnnotatedString(title), text, onClick)

@Composable
private fun Component(
    title: AnnotatedString, text: String?, onClick: () -> Unit
) {
    Box(
        modifier = Modifier
            .fillMaxWidth()
